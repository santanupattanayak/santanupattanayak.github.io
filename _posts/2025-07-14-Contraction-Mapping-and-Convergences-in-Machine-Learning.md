...
A **Complete Metric Space** is one in which every Cauchy sequence converges to the **limit** that lies within the metric space. In essence it's a space which contains all the limit points of every possible Cauchy sequence in the that metric space.
...
The sequence is Cauchy, however since $$e$$ is not a rational number hence the sequence cannot converge in the metric space of $$\mathbb{Q} $$. In essence $$\mathbb{Q} $$ is not a complete metric space. The sequence would have converged in the metric space $$\mathbb{R} $$ as $$e \in \mathbb{R} $$.  
For a sequence to converge the terms of the sequence have to get arbitrarily close to each other as the sequence progresses (it has to be a Cauchy sequence) and the limit of the sequence also needs to be in the metric space. So for convergence we desire a Complete metric space where all Cauchy sequences can converge.

## Revisiting the Contraction Mapping Theorem <a name="rcm"></a>

Now equipped with the notions of Cauchy Sequences and Complete metric spaces, we can prove the existence of the fixed point $$x$$ satisfying $$Tx = x$$ by showing that the contraction mapping $$T$$ leads to a Cauchy sequence.  We define the iterative sequence  
  $$
  \begin{align} 
  x_{n+1} = Tx_{n}
  \end{align} 
...
As $$m,n\to\infty$$, since $$c \lt 1$$ hence $$c^{n}\to0$$. This leads to $$c^{n}\lvert x_{m-n} - x_{0} \rvert \to 0$$ . This proves that the sequence $$\{x_{n}\}$$ generated by $$T$$ is a Cauchy sequence, which must converge to the **limit point** in the Complete metric space. It's **limit** is the unique fixed point of T.

In the rest of the Chapter we will try to prove the Convergence of iterative algorithms in Machine Learning using the Contraction Mapping Theorem. Specifically we will study the Convergence of Gradient Descent for a well known Convex function as well as the Convergence of the Value Function in Reinforcement learning using Contraction Mapping theorem.
