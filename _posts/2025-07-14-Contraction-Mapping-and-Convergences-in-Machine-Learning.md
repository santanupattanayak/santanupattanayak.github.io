...
As $$m,n\to\infty$$ since for the contraction mapping $$c \lt 1$$ hence $$c^{n}\lvert x_{m-n} - x_{0} \rvert \to 0$$ . This proves that the sequence $$x_{n}$$ generated by $$T$$ is a Cauchy sequence and since the Contraction Mapping is defined on a Complete metric space the Cauchy sequence should converge to the limit of the sequence which is a unique fixed point.
