...
3. Given a model class and training methodology each dataset $$D$$ would produce a different model $$f(y|x;\theta_{D})$$ and that's what lead to the variance of the model. Because of this variability for a given $x$ models trained on different datasets $$D$$ would produce different predictions $$\hat{y_{D}} = f(y|x;\theta_{D})$$ . The variance of the model can be represented in terms of the parameters of the model as $${\mathbb{E}}[(\theta_{D} - \mathbb{E}[\theta_{D}])^{2}]$$ or in terms of the variance of predictions over the different models given an input $$x$$ i.e.  $${\mathbb{E}}[(\hat{y_{D}} - \mathbb{E}[\hat{y_{D}}])^{2}]$$. We will choose the latter during our bias-variance decomposition.
