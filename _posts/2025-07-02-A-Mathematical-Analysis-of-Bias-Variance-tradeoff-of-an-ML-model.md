...
1. Whenever we build a model, instead of training the model with all the data possible under the distribution $$\doubleP(x,y)$$ we sample a small set of m points
sampled from $$\doubleP(x,y)$$ which we call as training dataset D. The training dataset $$D$$ of $$m$$ points can be represented  
