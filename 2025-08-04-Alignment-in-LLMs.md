...
In the context of the LLM alignment we assume that given a query $$x$$ the LLM which acts as a policy $$\pi_{\theta}$$ generates the output $$y$$ stochastically. Then as per PPO the optimization objective is as below 

$$
\begin{align}
\mathbb{E}_{x \sim D_x}\mathbb{E}_{y \sim \pi_{\theta}(y|x)}\left[r_{\phi}(x,y) - \beta.KL(\pi_{\theta}(y|x) || \pi_{\theta_{ref}}(y|x))\right] 
\end{align}
$$
